...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/faileffect.js"></script>
</head>
<body>
	<c:choose>
	<c:when test="${loginFail == true}">
		<div id="faileffect"></div>
		<script>fadeOutFailEffect();</script>
	</c:when>
	<c:when test="${loginFail == false}">
	</c:when>
	</c:choose>
	<div id="container">
